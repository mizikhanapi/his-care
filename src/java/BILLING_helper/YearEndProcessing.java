...
public class YearEndProcessing {
    ...
    public YearEndProcessing(String userID, String hfc, String dis, String subdis) {
    ...
    public int doBackUpData() {
        ...
        try {
            ...
            String sqlSelect = "SELECT code FROM far_year_end_parameter WHERE code = 'yep' AND hfc_cd = '" + hfc_cd + "' ";
            ArrayList<ArrayList<String>> dupl = conn.getData(sqlSelect);

            // Deleting old legger for this hfc
            String sqlDelete = "DELETE FROM far_customer_ledger_backup WHERE hfc_cd = '" + hfc_cd + "' ";
            ...
            if (dupl.isEmpty()) {
                ...
                String sqlInsert = "INSERT INTO far_year_end_parameter ( code, hfc_cd, process_status, processed_year, status, created_by , created_date) "
                        + " VALUES ('yep','" + hfc_cd + "','1','" + strYear + "','0','" + user_id + "','" + created_date + "') ";
                bool = rmic.setQuerySQL(conn.HOST, conn.PORT, sqlInsert);

            } else {

                String sqlUpdate = "UPDATE far_year_end_parameter SET process_status = '1'  WHERE code = 'yep' AND hfc_cd = '" + hfc_cd + "' ";
